...
import { Product, InventoryTransaction, StockTransfer, PriceBook, PriceOverride, Customer, LoyaltyTier, LoyaltyLog, Offer } from '@/types';
...
db.version(6).stores({
    ...
    priceOverrides: '[priceBookId+productId], [priceBookId+variantId], priceBookId'
});
...
export const syncProductsToLocal = async (products: Product[]) => {
    try {
        ...
        const tiers: LoyaltyTier[] = [
            { id: 'tier-bronze', name: 'Bronze', minSpend: 0, earnRate: 1, color: '#CD7F32' },
            { id: 'tier-silver', name: 'Silver', minSpend: 500, earnRate: 1.5, color: '#C0C0C0' },
            { id: 'tier-gold', name: 'Gold', minSpend: 2000, earnRate: 2, color: '#FFD700' }
        ];
...
export const updateLocalStock = async (transactions: InventoryTransaction[]) => {
    try {
        await db.transaction('rw', db.inventoryLevels, async () => {
            for (const tx of transactions) {
                // Dexie compound key get
                const level = await db.inventoryLevels.get({ productId: tx.productId, locationId: tx.locationId });
